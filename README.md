...
记忆构成一个多层次知识库，分为"工作流层面"和"执行层面"模式。
...
| AGENT KB | 分层知识库（工作流层+执行层）；以数据库形式保存对话与业务数据。 |
| MemAgent | 维护固定长度内部上下文记忆，不显式存储单条记忆，主要靠RL策略在线更新记忆窗口；没有外部结构化存储。 |
| Learn to Memorize | 构造混合检索记忆系统，可能以多种嵌入方式存储记忆条目；强调多信号检索（相关性、时序等），使用向量化记忆表。 |
...
| AGENT KB | 采用双阶段检索：学生代理从"工作流知识"中检索宏观策略，教师代理从"执行日志"中检索细节；无训练；检索结果由模型Refine融合；无多Agent分发，多框架共享知识库。 |
...
| 内容形态 | 核心思想 | 相关论文 |
| :--- | :--- | :--- |
| **原始/接近原始的轨迹** | 以最小的加工处理，储存完整的或接近完整的交互序列（如状态-动作-奖励元组），保真度最高。 | Memento |
| **多层次混合形态** | 同时储存原始或半处理的交互日志，以及从中提炼出的高层抽象知识（如技巧、洞见），兼顾细节与通用性。 | G-Memory, CFGM, Zep, Nemori |
| **结构化/半结构化知识** | 将经验加工成带有特定结构的形式，如工作流、带有元数据的笔记、图中的事实节点或层次化节点。 | AWM, Memp, AGENT KB, MemTree, MMS, A-MEM, Mem0 |
| **高度抽象/提炼的知识** | 丢弃原始轨迹，只储存经过LLM提炼、总结后的文本摘要、规则、事实或“想法”。 | CER, IML, Learn to Memorize, Memory-R1, TiM |
| **程序化知识** | 将可复用的操作流程直接编码为可执行的程序代码（如Python函数），实现了知识的行动化。 | ASI |
| **内部状态/压缩表征** | 记忆并非人类可读的文本，而是模型内部的、经过高度压缩的表示，如Token序列或KV缓存块。 | MemAgent, EM-LLM, MEM1 |


### 加工与处理分类
| 加工方法 | 核心思想 | 相关论文 |
| :--- | :--- | :--- |
| **基于LLM的蒸馏/归纳/反思** | 主要依赖大型语言模型（LLM）的强大理解和生成能力，通过设计不同的提示（Prompt）来对原始经验进行总结、归纳、反思，从而提炼出关键信息。 | CER, IML, AWM, Memp, Learn to Memorize, TiM, A-MEM, Mem0 |
| **程序化/结构化转换** | 将非结构化的交互轨迹转换为具有明确格式的、机器更易于处理的结构，如代码、形式化元组或知识图谱中的实体和关系。 | ASI, AGENT KB, Zep, CFGM |
| **在线压缩/状态更新** | 不储存离散的记忆条目，而是通过一个（通常由强化学习训练的）策略，持续地将新信息融合进一个固定大小的内部状态中，并丢弃冗余信息。 | MemAgent, MEM1 |
| **分层/动态聚合** | 将新信息整合进一个层次化结构（如树或多层图）中，并通过聚合操作更新更高层级的抽象节点，实现知识的动态组织。 | G-Memory, MemTree |
| **事件分割/认知处理** | 借鉴认知科学理论，将连续的经验流分割成有意义的“事件”单元，或模拟多层次的认知过程来提取不同维度的记忆片段。 | EM-LLM, MMS, Nemori |
| **几乎无处理** | 对原始的交互数据仅做最基础的格式化处理，基本保留其原始形态。 | Memento |
...
| G-Memory | 存储多轮协作的摘要和见解；原始轨迹抽象成图节点；检索时在记忆图中查找相关事件；多Agent共享一套记忆图，按ROLE个性分发Insight。 |
